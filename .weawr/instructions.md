...
- Add only the dependencies your issue names, plus dev tooling the issue implies (types packages, `tsx`). Anything else, ask. `@modelcontextprotocol/server` is added only by Slice 5's issue #1 (the `/mcp` endpoint); later issues use it, they don't add it again. Slice 6 (the Web UI): `tailwindcss`, `@tailwindcss/vite`, shadcn's generated components and their Radix dependencies, `playwright` and `@axe-core/playwright` are added only by its issue #1, and `@scalar/api-reference` only by its issue #4; later issues use them. UI work follows `PRODUCT.md` and `DESIGN.md` at the repo root, and extends the shell and Search (Impeccable is in `.claude/skills/impeccable/`).
