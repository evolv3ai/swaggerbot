...
- **Umbrella names** (checked before `whichApi`): when `normalizeName(name)` equals the first label of a Vendor id (`google` ↔ `google.com`, `azure` ↔ `azure.com`) and two or more Candidates belong to that Vendor, the Outcome is Ambiguous, listing up to 10 of them (APIs.guru order) with equal probabilities, and `whichApi` is not asked. Why: live Jev answers `none` at 0.98 for "google" over Drive and Gmail, which the rule above would turn into Unknown. One Candidate from a matching Vendor is not enough; it goes through `whichApi` like any other.
