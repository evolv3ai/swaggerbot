...
import type { Outcome, SpecAnswer } from "~/domain/outcome";
...
The Spec itself is never returned inline: the result gives its download URLs, the Published Form (the Spec as the Vendor serves it) and the Normalized Form (one bundled document in the current OpenAPI version). The result's text starts with what was found and the call to make next.
...
export function lookupResult(
  ...
  if (answer.status === 200)
    return {
      content: [{ type: "text", text: outcomeText(request, answer.body) }],
...
/** One sentence (or a few) for the agent: what the Outcome is and what to call next. */
export function outcomeText(request: LookupRequest, outcome: Outcome): string {
  switch (outcome.outcome) {
    case "Resolved": {
      ...
      return [
        ...
        alternates > 0
          ...
          : "",
        `Next: get_spec_outline(apiId: "${api.id}") to find the operation you need.`,
      ]
        .filter(Boolean)
        .join(" ");
    }
    case "Ambiguous": {
      const names = outcome.candidates
        ...
        .join(", ");
      return `Ambiguous: "${request.name}" could mean several APIs. Next: lookup_api again with one of these names: ${names}.`;
    }
    case "Unconfirmed": {
      ...
      return [
        ...
        downloadsText(spec),
        `Check it before relying on it; get_spec_outline(apiId: "${api.id}") shows what it covers.`,
      ].join(" ");
    }
    case "NoSpec":
      return `No Spec: ${outcome.api.name} by ${outcome.vendor.name}, apiId "${outcome.api.id}", has no Spec at an allowed Provenance.${
        outcome.communityAvailable
          ? ` A Community Spec exists. Next: lookup_api(name: "${request.name}", allowCommunity: true) to get it.`
          : ""
      }`;
    case "Unknown":
      return `Unknown: no API called "${outcome.name}" was found. Check the name, or try the Vendor's name or the API's full name.`;
  }
}
