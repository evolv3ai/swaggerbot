...
/** The time all the external references of one Spec may take to fetch, together. */
export const FORMS_REF_BUDGET_MS = 30_000;
...
export function createFormsWorker({
  ...
  function refFetcher(): (url: string) => Promise<Uint8Array> {
    let budget: AbortSignal | undefined;
    return async (url) => {
      budget ??= AbortSignal.timeout(FORMS_REF_BUDGET_MS);
      return (await fetcher.fetchUrl(url, { signal: budget })).bytes;
    };
  }
